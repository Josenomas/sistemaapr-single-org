...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        // Crear el procedimiento con filtro de organización
        DB::unprepared("CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_generar_boletas_mes`(
            IN `p_mes` VARCHAR(7),
...
BEGIN
    ...
    -- Cursor para socios FILTRADO POR ORGANIZACIÓN
    ...
    -- Cursor para tramos de tarifas FILTRADO POR ORGANIZACIÓN
    ...
    read_loop: LOOP
        ...
        -- Buscar lectura FILTRADA POR ORGANIZACIÓN
        ...
        -- Verificar si ya existe boleta para este socio y mes EN ESTA ORGANIZACIÓN
        IF EXISTS (
        ...
        -- Insertar boleta con id_organizacion
        INSERT INTO boletas (
            id_organizacion,
            numero_boleta,
            id_socio,
    ...
    -- Retornar cantidad de boletas generadas SOLO DE ESTA ORGANIZACIÓN
...
END");
    ...
    {
        // No hay down, el SP anterior ya no es compatible
        DB::unprepared("DROP PROCEDURE IF EXISTS sp_generar_boletas_mes");
    }
};
