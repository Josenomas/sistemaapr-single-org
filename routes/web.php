...
Route::middleware('auth')->group(function () {
    ...
    // ========================================
    // DEBUG: Limpiar boletas huérfanas
    // ========================================
    Route::get('/debug/boletas-huerfanas/{mes}', function($mes) {
        $boletas = \App\Models\Boleta::where('mes', $mes)
            ->where('activo', 1)
            ->with('socio')
            ->get();
        ...
        foreach ($boletas as $boleta) {
            if (!$boleta->socio ||
                $boleta->socio->activo == 0 ||
                $boleta->socio->estado == 'desconectado') {
                $huerfanas[] = [
                    'id' => $boleta->id,
                    'numero_boleta' => $boleta->numero_boleta,
                    'id_socio' => $boleta->id_socio,
                    'socio_numero' => $boleta->socio?->numero_socio ?? 'N/A',
                    'socio_nombre' => $boleta->socio?->nombre_completo ?? 'SOCIO ELIMINADO',
                    'socio_estado' => $boleta->socio?->estado ?? 'N/A',
                    'socio_activo' => $boleta->socio?->activo ?? 0,
                ];
            }
        }
        ...
        return response()->json([
            ...
            'boletas_huerfanas' => count($huerfanas),
            'detalle' => $huerfanas,
        ]);
    })->name('debug.boletas.huerfanas');

    Route::post('/debug/limpiar-boletas-huerfanas/{mes}', function($mes) {
        $boletas = \App\Models\Boleta::where('mes', $mes)
            ->where('activo', 1)
            ->with('socio')
            ->get();
        ...
        return response()->json([
            'mensaje' => 'Boletas huérfanas eliminadas',
            'cantidad_eliminadas' => count($eliminadas),
            'detalle' => $eliminadas,
        ]);
    })->name('debug.limpiar.boletas.huerfanas');
