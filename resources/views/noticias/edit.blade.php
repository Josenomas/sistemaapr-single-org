...
@section('title', 'Editar Noticia - Sistema APR')
    ...
    <div class="card-body">
        <form action="{{ route('noticias.update', $noticia->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select class="form-control @error('categoria') is-invalid @enderror"
                        ...
                        <option value="Aviso" {{ old('categoria', $noticia->categoria) == 'Aviso' ? 'selected' : '' }}>Aviso</option>
                        <option value="Evento" {{ old('categoria', $noticia->categoria) == 'Evento' ? 'selected' : '' }}>Evento</option>
                        <option value="Mantenimiento" {{ old('categoria', $noticia->categoria) == 'Mantenimiento' ? 'selected' : '' }}>Mantenimiento</option>
                        <option value="Corte" {{ old('categoria', $noticia->categoria) == 'Corte' ? 'selected' : '' }}>Corte</option>
                        <option value="Reunión" {{ old('categoria', $noticia->categoria) == 'Reunión' ? 'selected' : '' }}>Reunión</option>
                        <option value="Otro" {{ old('categoria', $noticia->categoria) == 'Otro' ? 'selected' : '' }}>Otro</option>
                    </select>
                    @error('categoria')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date"
                           ...
                           value="{{ old('fecha_publicacion', $noticia->fecha_publicacion ? $noticia->fecha_publicacion->format('Y-m-d') : date('Y-m-d')) }}">
                ...
                <div class="form-group col-md-4">
                    ...
                    <div class="checkbox-wrapper">
                        <label class="checkbox-item">
                            <input type="checkbox" name="destacada" value="1" {{ old('destacada', $noticia->destacada) ? 'checked' : '' }}>
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control @error('resumen') is-invalid @enderror"
                              ...
                              name="resumen"
                              rows="2"
                              placeholder="Descripción corta que aparecerá en el listado (máximo 500 caracteres)">{{ old('resumen', $noticia->resumen) }}</textarea>
                    @error('resumen')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-row">
                <!-- Imagen Destacada -->
                <div class="form-group col-md-12">
                    <label for="imagen_destacada" class="form-label">{{ $noticia->imagen_destacada ? 'Cambiar Imagen Destacada' : 'Imagen Destacada' }}</label>
