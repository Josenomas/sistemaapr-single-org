...
@section('title', 'Editar Sueldo - Sistema APR')
...
<div class="page-header">
    ...
    <a href="{{ route('sueldos.show', $sueldo->id) }}" class="btn btn-secondary">
...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('sueldos.update', $sueldo->id) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="form-group full-width">
                        ...
                        <select id="id_funcionario" name="id_funcionario" class="form-control @error('id_funcionario') error @enderror" required>
                            ...
                            @foreach($funcionarios as $funcionario)
                                <option value="{{ $funcionario->id }}" {{ old('id_funcionario', $sueldo->id_funcionario) == $funcionario->id ? 'selected' : '' }}>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="periodo" name="periodo" class="form-control @error('periodo') error @enderror"
                               value="{{ old('periodo', $sueldo->periodo) }}" placeholder="2024-01" pattern="\d{4}-\d{2}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="fecha_pago" name="fecha_pago"
                               ...
                               value="{{ old('fecha_pago', $sueldo->fecha_pago->format('Y-m-d')) }}" required>
                        @error('fecha_pago')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="sueldo_base" name="sueldo_base"
                               ...
                               value="{{ old('sueldo_base', $sueldo->sueldo_base) }}" min="0" step="0.01" required
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="bonos" name="bonos"
                               ...
                               value="{{ old('bonos', $sueldo->bonos) }}" min="0" step="0.01"
                               onchange="calcularTotal()">
                        @error('bonos')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="descuentos" name="descuentos"
                               ...
                               value="{{ old('descuentos', $sueldo->descuentos) }}" min="0" step="0.01"
                               onchange="calcularTotal()">
                        @error('descuentos')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="total-liquido-display" id="total_liquido_display">
                            {{ $sueldo->total_liquido_formateado }}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="metodo_pago" name="metodo_pago" class="form-control @error('metodo_pago') error @enderror" required>
                            ...
                            <option value="efectivo" {{ old('metodo_pago', $sueldo->metodo_pago) == 'efectivo' ? 'selected' : '' }}>Efectivo</option>
                            <option value="transferencia" {{ old('metodo_pago', $sueldo->metodo_pago) == 'transferencia' ? 'selected' : '' }}>Transferencia</option>
                            <option value="cheque" {{ old('metodo_pago', $sueldo->metodo_pago) == 'cheque' ? 'selected' : '' }}>Cheque</option>
                        </select>
                        @error('metodo_pago')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select id="estado" name="estado" class="form-control @error('estado') error @enderror" required>
                            <option value="pendiente" {{ old('estado', $sueldo->estado) == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                            <option value="pagado" {{ old('estado', $sueldo->estado) == 'pagado' ? 'selected' : '' }}>Pagado</option>
                            <option value="anulado" {{ old('estado', $sueldo->estado) == 'anulado' ? 'selected' : '' }}>Anulado</option>
                        </select>
                        @error('estado')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <input type="text" id="comprobante" name="comprobante"
                               ...
                               value="{{ old('comprobante', $sueldo->comprobante) }}" placeholder="Ej: TRANS-123456, CHQ-001">
                        @error('comprobante')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <textarea id="observaciones" name="observaciones" class="form-control @error('observaciones') error @enderror"
                                  rows="4">{{ old('observaciones', $sueldo->observaciones) }}</textarea>
                        @error('observaciones')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
