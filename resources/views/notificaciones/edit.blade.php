...
@section('title', 'Editar Notificación - Sistema APR')
...
<div class="page-header">
    ...
    <a href="{{ route('notificaciones.show', $notificacion->id) }}" class="btn btn-secondary">
...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('notificaciones.update', $notificacion->id) }}" method="POST" id="formNotificacion">
            @csrf
            @method('PUT')
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control @error('tipo') is-invalid @enderror"
                        ...
                        <option value="informativa" {{ old('tipo', $notificacion->tipo) == 'informativa' ? 'selected' : '' }}>Informativa</option>
                        <option value="recordatorio" {{ old('tipo', $notificacion->tipo) == 'recordatorio' ? 'selected' : '' }}>Recordatorio</option>
                        <option value="urgente" {{ old('tipo', $notificacion->tipo) == 'urgente' ? 'selected' : '' }}>Urgente</option>
                        <option value="corte" {{ old('tipo', $notificacion->tipo) == 'corte' ? 'selected' : '' }}>Corte de Servicio</option>
                        <option value="reunion" {{ old('tipo', $notificacion->tipo) == 'reunion' ? 'selected' : '' }}>Reunión</option>
                    </select>
                    @error('tipo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control @error('destinatario') is-invalid @enderror"
                            ...
                            required>
                        <option value="">Seleccione</option>
                        <option value="todos" {{ old('destinatario', $notificacion->destinatario) == 'todos' ? 'selected' : '' }}>Todos los Socios</option>
                        <option value="socio" {{ old('destinatario', $notificacion->destinatario) == 'socio' ? 'selected' : '' }}>Socio Específico</option>
                        <option value="sector" {{ old('destinatario', $notificacion->destinatario) == 'sector' ? 'selected' : '' }}>Por Sector</option>
                    </select>
                    @error('destinatario')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control @error('estado') is-invalid @enderror"
                            ...
                            required>
                        <option value="">Seleccione</option>
                        <option value="borrador" {{ old('estado', $notificacion->estado) == 'borrador' ? 'selected' : '' }}>Borrador</option>
                        <option value="programada" {{ old('estado', $notificacion->estado) == 'programada' ? 'selected' : '' }}>Programada</option>
                        <option value="enviada" {{ old('estado', $notificacion->estado) == 'enviada' ? 'selected' : '' }}>Enviar Ahora</option>
                    </select>
                    @error('estado')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-6" id="socio-field" style="display: none;">
                    ...
                    <select class="form-control @error('id_socio') is-invalid @enderror"
                        ...
                        @foreach($socios as $socio)
                            <option value="{{ $socio->id }}" {{ old('id_socio', $notificacion->id_socio) == $socio->id ? 'selected' : '' }}>
                ...
                <div class="form-group col-md-6" id="sector-field" style="display: none;">
                    ...
                    <input type="text"
                           ...
                           id="sector"
                           name="sector"
                           value="{{ old('sector', $notificacion->sector) }}"
                           maxlength="100"
                ...
                <div class="form-group col-md-6">
                    ...
                    <input type="datetime-local"
                           ...
                           value="{{ old('fecha_programada', $notificacion->fecha_programada ? $notificacion->fecha_programada->format('Y-m-d\TH:i') : '') }}">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    ...
                    <div class="checkbox-group">
                        @php
                            $canales = old('canal', is_array($notificacion->canal) ? $notificacion->canal : (is_string($notificacion->canal) ? json_decode($notificacion->canal, true) : []));
                            if (!is_array($canales)) {
                                $canales = [];
                            }
                        @endphp
                        <label class="checkbox-label">
                            <input type="checkbox"
                                   name="canal[]"
                                   value="email"
                                   {{ in_array('email', $canales) ? 'checked' : '' }}>
                            <span>Email</span>
                        </label>
                        <label class="checkbox-label">
                            <input type="checkbox"
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control @error('observaciones') is-invalid @enderror"
                              id="observaciones"
                              name="observaciones"
                              rows="3">{{ old('observaciones', $notificacion->observaciones) }}</textarea>
                    @error('observaciones')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
