...
@section('title', 'Editar Vacación - Sistema APR')
        ...
        <form action="{{ route('vacaciones.update', $vacacion->id) }}" method="POST">
            ...
            <div class="form-row">
                    ...
                    <select name="id_funcionario" id="id_funcionario" class="form-control @error('id_funcionario') is-invalid @enderror" required>
                        ...
                        @foreach($funcionarios as $funcionario)
                            <option value="{{ $funcionario->id }}"
                                {{ (old('id_funcionario', $vacacion->id_funcionario) == $funcionario->id) ? 'selected' : '' }}>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="text" name="periodo" id="periodo" class="form-control @error('periodo') is-invalid @enderror"
                           value="{{ old('periodo', $vacacion->periodo) }}" required maxlength="4" placeholder="Ej: 2024">
                    @error('periodo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="fecha_inicio" id="fecha_inicio"
                           ...
                           value="{{ old('fecha_inicio', $vacacion->fecha_inicio->format('Y-m-d')) }}" required>
                    @error('fecha_inicio')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="fecha_termino" id="fecha_termino"
                           ...
                           value="{{ old('fecha_termino', $vacacion->fecha_termino->format('Y-m-d')) }}" required>
                    @error('fecha_termino')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="number" name="dias_habiles" id="dias_habiles"
                           ...
                           value="{{ old('dias_habiles', $vacacion->dias_habiles) }}" required min="1" placeholder="Ej: 15">
                    @error('dias_habiles')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <select name="tipo" id="tipo" class="form-control @error('tipo') is-invalid @enderror" required>
                        ...
                        <option value="legales" {{ old('tipo', $vacacion->tipo) == 'legales' ? 'selected' : '' }}>Legales</option>
                        <option value="progresivas" {{ old('tipo', $vacacion->tipo) == 'progresivas' ? 'selected' : '' }}>Progresivas</option>
                        <option value="administrativas" {{ old('tipo', $vacacion->tipo) == 'administrativas' ? 'selected' : '' }}>Administrativas</option>
                        <option value="sin_goce" {{ old('tipo', $vacacion->tipo) == 'sin_goce' ? 'selected' : '' }}>Sin Goce de Sueldo</option>
                    </select>
                    @error('tipo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="estado" id="estado" class="form-control @error('estado') is-invalid @enderror" required>
                        <option value="">Seleccione...</option>
                        <option value="solicitada" {{ old('estado', $vacacion->estado) == 'solicitada' ? 'selected' : '' }}>Solicitada</option>
                        <option value="aprobada" {{ old('estado', $vacacion->estado) == 'aprobada' ? 'selected' : '' }}>Aprobada</option>
                        <option value="rechazada" {{ old('estado', $vacacion->estado) == 'rechazada' ? 'selected' : '' }}>Rechazada</option>
                        <option value="en_curso" {{ old('estado', $vacacion->estado) == 'en_curso' ? 'selected' : '' }}>En Curso</option>
                        <option value="finalizada" {{ old('estado', $vacacion->estado) == 'finalizada' ? 'selected' : '' }}>Finalizada</option>
                        <option value="cancelada" {{ old('estado', $vacacion->estado) == 'cancelada' ? 'selected' : '' }}>Cancelada</option>
                    </select>
                    @error('estado')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="suplente" id="suplente" class="form-control @error('suplente') is-invalid @enderror">
                        ...
                        @foreach($funcionarios as $funcionario)
                            <option value="{{ $funcionario->id }}"
                                {{ (old('suplente', $vacacion->suplente) == $funcionario->id) ? 'selected' : '' }}>
            ...
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="fecha_solicitud" id="fecha_solicitud"
                           ...
                           value="{{ old('fecha_solicitud', $vacacion->fecha_solicitud->format('Y-m-d')) }}" required>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="id_aprobador" id="id_aprobador" class="form-control @error('id_aprobador') is-invalid @enderror">
                        ...
                        @foreach($funcionarios as $funcionario)
                            <option value="{{ $funcionario->id }}"
                                {{ (old('id_aprobador', $vacacion->id_aprobador) == $funcionario->id) ? 'selected' : '' }}>
                                {{ $funcionario->nombre }} {{ $funcionario->apellido_paterno }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="fecha_aprobacion" id="fecha_aprobacion"
                           ...
                           value="{{ old('fecha_aprobacion', $vacacion->fecha_aprobacion ? $vacacion->fecha_aprobacion->format('Y-m-d') : '') }}">
            ...
            <div class="form-group">
                ...
                <textarea name="motivo_rechazo" id="motivo_rechazo" rows="2"
                          ...
                          placeholder="Solo si fue rechazada">{{ old('motivo_rechazo', $vacacion->motivo_rechazo) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea name="observaciones" id="observaciones" rows="3"
                          ...
                          placeholder="Notas adicionales sobre la vacación...">{{ old('observaciones', $vacacion->observaciones) }}</textarea>
                @error('observaciones')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
