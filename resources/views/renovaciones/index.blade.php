...
@section('title', 'Renovaciones de Medidores - Sistema APR')
...
<div class="card">
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('renovaciones.index') }}" class="filter-form">
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select name="motivo" id="motivo" class="form-control">
                        ...
                        <option value="deterioro" {{ request('motivo') == 'deterioro' ? 'selected' : '' }}>Deterioro</option>
                        <option value="falla" {{ request('motivo') == 'falla' ? 'selected' : '' }}>Falla</option>
                        <option value="actualizacion" {{ request('motivo') == 'actualizacion' ? 'selected' : '' }}>Actualización</option>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Listado de Renovaciones</h3>
        <div class="card-stats">
            Total: <strong>{{ $renovaciones->total() }}</strong> registros
        </div>
    </div>
    <div class="card-body">
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach($renovaciones as $renovacion)
                            <tr>
                                <td><strong>{{ $renovacion->socio->numero_socio }}</strong></td>
                                <td>{{ $renovacion->socio->nombre_completo }}</td>
                                <td>{{ $renovacion->medidor_anterior ?? '-' }}</td>
                                <td><strong>{{ $renovacion->medidor_nuevo }}</strong></td>
                                <td>{{ $renovacion->fecha_renovacion->format('d/m/Y') }}</td>
                                <td>
                                ...
                                <td>{{ $renovacion->tecnico ? $renovacion->tecnico->nombre_completo : '-' }}</td>
                                <td>
                                    <div class="action-buttons">
                                        <a href="{{ route('renovaciones.show', $renovacion->id) }}" class="btn-icon" title="Ver detalle">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('renovaciones.edit', $renovacion->id) }}" class="btn-icon" title="Editar">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form method="POST" action="{{ route('renovaciones.destroy', $renovacion->id) }}"
                                              onsubmit="return confirm('¿Está seguro de eliminar esta renovación?')" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            <div class="empty-state">
                <i class="fas fa-sync-alt"></i>
                <p>No se encontraron renovaciones de medidores</p>
                <a href="{{ route('renovaciones.create') }}" class="btn btn-primary">
                    <i class="fas fa-plus"></i>
                    Registrar Primera Renovación
                </a>
            </div>
        @endif
    </div>
