...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: Arial, sans-serif; font-size: 11px; color: #1a1a1a; background: white; padding: 16px; width: 100%; max-width: 720px; margin: 0 auto; }
...
@php
    ...
    $subtotalInicial = $boleta->cargo_consumo + $boleta->cargo_fijo + ($boleta->otros_cargos ?? 0);
    ...
    // Calcular porcentaje de consumo para barra
    $consumoMaximo = 18; // Puedes hacerlo dinámico si tienes este dato
    $porcentajeConsumo = min(100, ($boleta->consumo_m3 / $consumoMaximo) * 100);

    // Calcular promedio de consumo del historial
    $promedioConsumo = $historialConsumo && $historialConsumo->count() > 0
        ? $historialConsumo->avg('consumo')
        : 0;
@endphp
...
<div class="header">
  ...
  <div class="folio-box">
    ...
    <div class="folio-sub">Folio SII: {{ $boleta->folio_sii }}</div>
    @endif
  </div>
</div>
...
<!-- META -->
<div class="meta">
  <div class="meta-item"><div class="label">Período facturado</div><div class="value">{{ $boleta->mes_texto }}</div></div>
  <div class="meta-item"><div class="label">Fecha emisión</div><div class="value">{{ $boleta->fecha_emision_formateada }}</div></div>
  <div class="meta-item"><div class="label">Fecha vencimiento</div><div class="value">{{ $boleta->fecha_vencimiento_formateada }}</div></div>
</div>
...
<div class="two-col">
  <div class="section">
    ...
    <div class="row"><span class="muted">Nombre</span><span style="text-align:right;max-width:200px">{{ $boleta->socio->nombre_completo }}</span></div>
  ...
  <div class="section">
    ...
    <div class="bar-label">
      <span>0 m³</span>
      <span style="font-weight:600;color:#1a1a1a">{{ number_format($boleta->consumo_m3, 0) }} m³ actuales</span>
      <span>{{ $consumoMaximo }} m³ máx.</span>
    </div>
...
<div class="chart-wrap">
  ...
  <svg width="688" height="90" viewBox="0 0 688 90" xmlns="http://www.w3.org/2000/svg">
    @php
        $maxConsumo = max($historialConsumo->max('consumo'), 10);
        $barWidth = 60;
        $spacing = 12;
    ...
    @foreach($historialConsumo as $index => $item)
        @php
            ...
            $barHeight = ($item['consumo'] / $maxConsumo) * $maxHeight;
            $y = $baseY - $barHeight;
            ...
            // Formatear mes corto
            $mesTexto = substr($item['mes_texto'], 0, 3);
        @endphp

        <rect x="{{ $x }}" y="{{ $y }}" width="{{ $barWidth }}" height="{{ $barHeight }}" rx="2" fill="{{ $fill }}" stroke="{{ $stroke }}" stroke-width="{{ $strokeWidth }}"/>
        <text x="{{ $x + ($barWidth / 2) }}" y="{{ $y - 3 }}" font-size="8.5" font-weight="{{ $esActual ? 'bold' : 'normal' }}" fill="{{ $esActual ? '#0F6E56' : '#555' }}" text-anchor="middle">{{ number_format($item['consumo'], 0) }}</text>
  ...
  <div class="chart-legend">
    ...
    <div class="legend-item"><div class="legend-dot" style="background:#1D9E75"></div>Período actual ({{ $boleta->mes_texto }})</div>
    @if($promedioConsumo > 0)
...
<div class="two-col">
  <div class="section">
    ...
    <div class="row"><span class="muted">Consumo agua ({{ number_format($boleta->consumo_m3, 0) }} m³)</span><span>{{ $boleta->cargo_consumo_formateado }}</span></div>
    <div class="row"><span class="muted">Cargo fijo mensual</span><span>{{ $boleta->cargo_fijo_formateado }}</span></div>
    @if(($boleta->otros_cargos ?? 0) > 0)
    <div class="row"><span class="muted">Otros cargos</span><span>{{ $boleta->otros_cargos_formateado }}</span></div>
    @endif
    @if($monto_subsidio > 0)
    <div class="row" style="color:#1565c0"><span class="muted">Subsidio ({{ $subsidio_porcentaje }}%)</span><span>-${{ number_format($monto_subsidio, 0, ',', '.') }}</span></div>
    @endif
    @if($monto_descuento > 0)
    <div class="row" style="color:#1565c0"><span class="muted">Descuento fijo</span><span>-${{ number_format($monto_descuento, 0, ',', '.') }}</span></div>
    @endif
    <div class="row"><span class="muted">Subtotal</span><span>${{ number_format($subtotal, 0, ',', '.') }}</span></div>
    <div class="row"><span class="muted">IVA (19%){{ $exentoIva ? ' - Exento' : '' }}</span><span>{{ $exentoIva ? 'Exento' : '$' . number_format($iva, 0, ',', '.') }}</span></div>
    <div class="row total"><span>Total a pagar</span><span>${{ number_format($totalConIva, 0, ',', '.') }}</span></div>
  </div>
  <div class="section">
    <div class="section-title">Estado de cuenta</div>
    @if($mesesAdeudados > 0)
      <div style="margin-bottom:5px"><span class="badge badge-warning">{{ $mesesAdeudados }} {{ $mesesAdeudados == 1 ? 'mes pendiente' : 'meses pendientes' }}</span></div>
      @foreach($boletasPendientes->take(8) as $boletaPendiente)
      <div class="row"><span class="muted">{{ $boletaPendiente->mes_texto }}</span><span>${{ number_format($boletaPendiente->total, 0, ',', '.') }}</span></div>
      @endforeach
      ...
      <div style="border-top: 0.5px solid #e0e0e0; padding-top: 4px; margin-top: 4px;"></div>
      <div class="row"><span class="muted">Último pago</span><span>{{ $boleta->ultimo_pago->fecha->format('d/m/Y') }}</span></div>
      <div class="row"><span class="muted">Método</span><span>{{ $boleta->ultimo_pago->metodo }}</span></div>
    @else
    ...
    @endif
    @if($ultimoPago)
    <div class="row" style="margin-top:3px"><span class="muted">Último pago {{ \Carbon\Carbon::parse($ultimoPago->fecha_pago)->format('d/m/Y') }}</span><span>${{ number_format($ultimoPago->monto_pagado, 0, ',', '.') }}</span></div>
    <div class="row"><span class="muted">Método</span><span>{{ ucfirst($ultimoPago->metodo_pago ?? 'efectivo') }}</span></div>
    @endif
  </div>
</div>
  ...
  <div class="pago-box">
    ...
    <p>
      @if($organizacion->banco && $organizacion->numero_cuenta)
        ...
        @endif
      @endif
      RUT: {{ $organizacion->rut }}<br>{{ $organizacion->email_contacto ?? 'sistemaapr@gmail.com' }}<br>Ref.: <strong>{{ $boleta->numero_boleta }}</strong>
    </p>
  </div>
</div>

<!-- TIMBRE SII (si aplica) -->
@if($boleta->tieneFolioSII())
<div class="timbre-section">
  ...
  <div>
    ...
    <div class="t-row">
      ...
      <div><div class="t-label">Monto</div><div class="t-val">${{ number_format($totalConIva, 0, ',', '.') }}</div></div>
    </div>
    <div class="t-legal">Documento válido sin firma ni timbre físico. Verifique en www.sii.cl</div>
  </div>
</div>
@endif
...
<!-- FOOTER -->
<div class="footer">
  <span>Generado electrónicamente {{ now()->format('d/m/Y H:i:s') }}</span>
  <span class="sii-badge">{{ $organizacion->nombre_apr }}</span>
</div>
