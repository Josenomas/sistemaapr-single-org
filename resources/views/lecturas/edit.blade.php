...
@section('title', 'Editar Lectura - Sistema APR')
...
<div class="page-header">
    ...
    <a href="{{ route('lecturas.show', $lectura->id) }}" class="btn btn-secondary">
        ...
        <form action="{{ route('lecturas.update', $lectura->id) }}" method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select class="form-control @error('id_socio') is-invalid @enderror"
                        ...
                        @foreach($socios as $socio)
                            <option value="{{ $socio->id }}" {{ old('id_socio', $lectura->id_socio) == $socio->id ? 'selected' : '' }}>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="month"
                           ...
                           id="mes"
                           name="mes"
                           value="{{ old('mes', date('Y-m', strtotime($lectura->mes))) }}"
                           required>
                    @error('mes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date"
                           ...
                           id="fecha_lectura"
                           name="fecha_lectura"
                           value="{{ old('fecha_lectura', date('Y-m-d', strtotime($lectura->fecha_lectura))) }}"
                           required>
                    @error('fecha_lectura')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('observaciones') is-invalid @enderror"
                          ...
                          name="observaciones"
                          rows="3"
                          placeholder="Notas adicionales sobre la lectura...">{{ old('observaciones', $lectura->observaciones) }}</textarea>
                @error('observaciones')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
